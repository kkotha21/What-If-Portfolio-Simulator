...
export default function App() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.get(`http://localhost:5001/api/investment`, {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '700px', margin: '0 auto' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '1rem' }}>
        <input type="text" placeholder="Ticker (e.g. AAPL)" value={ticker} onChange={e => setTicker(e.target.value)} required />
        <input type="number" placeholder="Amount (e.g. 1000)" value={amount} onChange={e => setAmount(e.target.value)} required />
        <input type="date" value={buyDate} onChange={e => setBuyDate(e.target.value)} required />
        <input type="date" value={sellDate} onChange={e => setSellDate(e.target.value)} />
        <button type="submit" style={{ padding: '0.5rem', backgroundColor: '#2563EB', color: 'white', border: 'none', borderRadius: '5px' }}>Simulate</button>
      </form>
      ...
      {result && (
        <div style={{ marginTop: '2rem' }}>
          ...
          {result.isPrediction && (
            <p style={{ color: 'orange' }}>Note: This is a predicted result based on historical trends and may not reflect actual future performance.</p>
          )}
          <Line
            data={{
              labels: result.dates,
